#!/usr/bin/env bash
# ============================================================================
# swa-start.sh — Build, start, and verify SWA CLI + Azure Functions
#
# This is the single entry point for starting SWA in environments without
# VS Code tasks (e.g. GitHub Actions agent sessions).
#
# Usage:
...
wait_for_ready() {
  ...
  while (( elapsed < TIMEOUT )); do
    ...
    if $swa_ready && $functions_ready && ! $api_ready; then
      local status
      status=$(curl -s -o /dev/null -w '%{http_code}' --max-time 5 "${SWA_URL}/api/auth/mode" 2>/dev/null) || status="000"
    ...
    if (( elapsed % 15 == 0 )); then
      log "  ⏳ ${elapsed}s — SWA:$(if $swa_ready; then echo '✓'; else echo '✗'; fi) Functions:$(if $functions_ready; then echo '✓'; else echo '✗'; fi) API:$(if $api_ready; then echo '✓'; else echo '✗'; fi)"
    fi
